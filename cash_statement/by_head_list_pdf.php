...
$str .= 'Date  : ' . $_REQUEST['transaction_date2'] . ' <br>';
$str .= 'As on : ' . $arr_year[0]['name'] . ' , From ' . $arr_year[0]['from_date'] . ' To  ' . $arr_year[0]['to_date'] . '<br>';

if (isset($_REQUEST['transaction_date2'])) {
    $where = " AND transaction_date='" . $_REQUEST['transaction_date2'] . "'";
}
...
for ($k = 0; $k < count($arr2); $k ++) {
    $debit = 0;
    $credit = 0;
    $balance = get_opening_balance_by_head($db, $arr2[$k]['head_id'], $_REQUEST['transaction_date2']);
    ...
    $info["where"] = "1 AND head_id='" . $arr2[$k]['head_id'] . "'  AND transaction_date='" . $_REQUEST['transaction_date2'] . "'";
    $arr = $db->select($info);
    ...
    for ($i = 0; $i < count($arr); $i ++) {
        ...
        $info2['where'] = "1 AND id='" . $arr[$i]['to_account_id'] . "' LIMIT 0,1";
        $res2 = $db->select($info2);
