...
$str .= 'Date  : From ' . $_REQUEST['transaction_date_from'] . ' To' . $_REQUEST['transaction_date_to'] . ' <br>';
...
for ($k = 0; $k < count($arr2); $k ++) {
    ...
    $balance = get_opening_balance_by_account($db, $arr2[$k]['account_id'], $_REQUEST['transaction_date_from']);
    ...
    $info["where"] = "1 AND account_id='" . $arr2[$k]['account_id'] . "'  AND transaction_date BETWEEN '" . $_REQUEST['transaction_date_from'] . "' AND '" . $_REQUEST['transaction_date_to'] . "'";
    $arr = $db->select($info);
