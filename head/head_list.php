...
<div class="portlet box green">
    ...
    ?>
								...
								<td nowrap="nowrap" align="right"><input type="hidden"
									name="cmd" id="cmd" value="search_head"> <input type="submit"
									name="btn_submit" id="btn_submit" value="Search" class="button" />
								</td>
							</TR>
						</table>
					</form>
			...
			<tr>
				<td><a href="head.php?cmd=edit" class="btn btn-primary">Add a head</a>
					<table
						class="table table-bordered table-striped table-condensed flip-content">
						<tr bgcolor="#ABCAE0">
							<td>Head Name</td>
...
for ($i = 0; $i < count($arr); $i ++) {
    ...
    ?>
			<tr bgcolor="<?=$row?>"
							...
							<td><?=$arr[$i]['head_name']?></td>
							<td><?=$arr[$i]['account_type']?></td>

							<td nowrap><a href="head.php?cmd=edit&id=<?=$arr[$i]['id']?>"
								class="btn default btn-xs purple"><i class="fa fa-edit"></i>Edit</a>
								<a href="head.php?cmd=delete&id=<?=$arr[$i]['id']?>"
